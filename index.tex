...
title: ``Manutenção e Confiabilidade: Aplicação em R'' author: ``Rafael
da Silva Fernandes'' date: last-modified format: html numbered: false
number-sections: false

Seja bem-vindo(a) a este livro técnico voltado à análise de
confiabilidade e estratégias de manutenção aplicadas a sistemas
industriais. A confiabilidade e a manutenção de ativos são disciplinas
essenciais para garantir a eficiência operacional, a segurança dos
processos e a sustentabilidade econômica de sistemas complexos.

Com o avanço das tecnologias de monitoramento, da coleta automatizada de
dados e das ferramentas de análise estatística, tornou-se possível
aplicar métodos quantitativos robustos para avaliar o desempenho de
equipamentos, prever falhas e otimizar políticas de manutenção.

Este material foi desenvolvido com o propósito de oferecer uma
introdução \textbf{prática e aplicada} aos conceitos de confiabilidade,
análise de dados de falha e modelagem de vida útil, com ênfase na
utilização da linguagem \textbf{R} como ferramenta principal. A
abordagem adotada combina fundamentos teóricos com aplicações reais,
permitindo ao leitor compreender os principais indicadores de
desempenho, ajustar modelos estatísticos e implementar estratégias de
manutenção baseadas em dados.

\section*{Indicadores-Chave de Desempenho
(ICDs)}\label{indicadores-chave-de-desempenho-icds}
\addcontentsline{toc}{section}{Indicadores-Chave de Desempenho (ICDs)}

\markright{Indicadores-Chave de Desempenho (ICDs)}

Ao longo do livro, serão abordados os principais indicadores utilizados
na gestão de manutenção:

\begin{itemize}
\item
  \textbf{DF -- Disponibilidade Física (Availability):} representa a
  capacidade de um sistema estar em condições de executar sua função.
  Sua indisponibilidade está diretamente relacionada às \textbf{Horas de
  Manutenção (HM)}, refletindo oportunidades de melhoria associadas ao
  status ``em manutenção''.
\item
  \textbf{UF -- Utilização Física:} indica o quanto da disponibilidade
  (\textbf{HD}) é efetivamente utilizada para operação (\textbf{HT}). A
  diferença entre HD e HT corresponde à \textbf{Ociosidade (HO)}, que
  ocorre quando o equipamento está ``liberado pela manutenção'' ou em
  ``parada operacional''.
\item
  \textbf{HT -- Horas Trabalhadas:} representam o tempo efetivo de
  operação dos ativos e são resultado direto da eficiência nos demais
  indicadores. Servem de base para o cálculo da UF e da produtividade.
\item
  \textbf{PR -- Produtividade:} expressa a eficiência do ciclo de
  carregamento e transporte, sendo calculada pela razão entre a
  \textbf{Massa Total transportada (MT)} e as \textbf{Horas Trabalhadas
  (HT)}. Está associada ao status ``apto para operação''.
\item
  \textbf{MTBF -- Mean Time Between Failures:} tempo médio entre falhas,
  mede a confiabilidade do equipamento ao indicar o intervalo médio de
  tempo em que o sistema opera sem falhas. Quanto maior o MTBF, maior a
  confiabilidade.
\item
  \textbf{MTTR -- Mean Time to Repair:} tempo médio para reparo,
  representa a eficiência da manutenção corretiva ao indicar quanto
  tempo, em média, é necessário para restaurar o funcionamento do
  equipamento após uma falha. Quanto menor o MTTR, maior a agilidade da
...
\tightlist
\item
  Engenheiros de confiabilidade e manutenção que buscam ferramentas
  práticas para análise de dados de falha e otimização de políticas.
\item
  Analistas de dados interessados em aplicar modelos estatísticos e de
  machine learning em contextos industriais.
\item
  Estudantes e pesquisadores que desejam aprender técnicas de análise de
  sobrevivência e modelagem de confiabilidade com exemplos reais.
\item
  Gestores técnicos que precisam tomar decisões baseadas em dados para
  melhorar a performance de ativos.
\end{itemize}
...
\tightlist
\item
  Compreender os conceitos fundamentais de confiabilidade,
  disponibilidade e manutenção.
\item
  Ajustar e interpretar distribuições de vida útil (Weibull,
  exponencial, lognormal) a dados reais.
\item
  Realizar análise de sobrevivência com dados censurados usando técnicas
  paramétricas e semiparamétricas.
\item
  Implementar estratégias de manutenção preventiva e preditiva com base
  em dados de sensores e histórico de falhas.
\item
  Utilizar simulação Monte Carlo para avaliar políticas de manutenção
  sob incerteza.
\item
  Aplicar boas práticas de gestão de dados e reprodutibilidade em
...
\tightlist
\item
  Instale o R e o RStudio (ou utilize o VS Code com extensões de R)
  conforme instruções no capítulo de instalação.
\item
  Execute os exemplos à medida que avança pelos capítulos para
  consolidar o aprendizado.
\item
  Explore os exercícios propostos ao final de cada capítulo para
  praticar os conceitos.
\item
  Utilize os dados de exemplo fornecidos na pasta
...
\chapter*{Como Usar este Livro}\label{como-usar-este-livro-1}
...
status de \textbf{``apto para operação''}, momento em que métricas de
análise e indicadores são estabelecidos para a otimização do processo.

Conforme ilustrado na Figure~\ref{fig-ciclo}, o ciclo inicia no
direcionamento do caminhão e segue a seguinte sequência de eventos:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{TVV (Tempo de Viagem Vazio):} O caminhão percorre vazio o
  trajeto até o ponto de carga.
\item
  \textbf{TFC (Tempo de Fila para Carga):} Aguarda na fila para ser
  carregado.
\item
  \textbf{TMC (Tempo de Manobra para Carregar):} Realiza a manobra para
  posicionamento.
\item
  \textbf{TC (Tempo de Carregamento):} Ocorre o carregamento pela
  escavadeira.
\item
  \textbf{TVC (Tempo de Viagem Cheio):} Transporte do material com o
  caminhão cheio.
\item
  \textbf{TFB (Tempo de Fila para Bascular):} Aguarda na fila do ponto
  de descarga.
\item
  \textbf{TB (Tempo de Basculamento):} Realiza o basculamento do
  material.
\end{enumerate}

Para um único turno de trabalho da operação, contabiliza-se o total de
ciclos realizados (\textbf{NC}).

\begin{figure}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{chapters/../images/fig-ciclo-processos.png}}

}

\caption{\label{fig-ciclo}Ciclo de processos de carregamento e
transporte.}

\end{figure}%

\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, coltitle=black, rightrule=.15mm, colback=white, breakable, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, opacityback=0, bottomtitle=1mm, opacitybacktitle=0.6, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, bottomrule=.15mm, left=2mm, toprule=.15mm, leftrule=.75mm, colframe=quarto-callout-note-color-frame]
...
\#\textbar{} label: fig-status-equipamento \#\textbar{} fig-cap:
``Transições entre status do equipamento'' \#\textbar{} echo: false
\#\textbar{} eval: false

library(DiagrammeR)

grViz('' digraph status \{ graph {[}rankdir = TB, bgcolor =
transparent{]}
...
``)
\pandocbounded{\includegraphics[keepaspectratio]{chapters/../images/fig-caminhao-797.png}}
...
\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, coltitle=black, rightrule=.15mm, colback=white, breakable, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Legenda das Categorias de Horas}, opacityback=0, bottomtitle=1mm, opacitybacktitle=0.6, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, bottomrule=.15mm, left=2mm, toprule=.15mm, leftrule=.75mm, colframe=quarto-callout-note-color-frame]
...
\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, coltitle=black, rightrule=.15mm, colback=white, breakable, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Interpretação das Perdas}, opacityback=0, bottomtitle=1mm, opacitybacktitle=0.6, arc=.35mm, colbacktitle=quarto-callout-important-color!10!white, toptitle=1mm, bottomrule=.15mm, left=2mm, toprule=.15mm, leftrule=.75mm, colframe=quarto-callout-important-color-frame]
